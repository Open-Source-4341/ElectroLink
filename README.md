...
| TS-24 | Endpoints de Datos de Consumo | Como desarrollador, quiero crear endpoints CRUD para gestionar datos de consumo eléctrico para permitir a propietarios registrar y consultar su información. | **Escenario 1: Registrar consumo** <br> Dado que se envía POST /api/consumption con datos de recibo <br> Cuando se procesa la petición <br> Entonces el sistema almacena los datos de consumo <br> Y calcula el costo total <br> Y retorna código 201 <br> <br> **Escenario 2: Consultar historial** <br> Dado que se envía GET /api/consumption con filtros de fecha <br> Cuando se procesa la petición <br> Entonces el sistema retorna lista de consumos filtrados <br> Y incluye totales calculados <br> Y retorna código 200 <br> <br> **Escenario 3: Calcular promedios** <br> Dado que se envía GET /api/consumption/stats <br> Cuando se procesa la petición <br> Entonces el sistema calcula promedios mensuales simples <br> Y retorna estadísticas básicas <br> Y retorna código 200 | EP-05 |
| TS-25 | Exportación de Datos | Como desarrollador, quiero crear un endpoint para exportar datos de consumo para permitir a usuarios descargar sus datos en formato CSV. | **Escenario 1: Exportar CSV** <br> Dado que se envía GET /api/consumption/export?format=csv <br> Cuando se procesa la petición <br> Entonces el sistema genera archivo CSV con headers <br> Y incluye todos los datos de consumo del usuario <br> Y retorna el archivo para descarga <br> <br> **Escenario 2: Exportar con filtros** <br> Dado que se envía petición con parámetros de fecha <br> Cuando se aplican los filtros <br> Entonces el sistema incluye solo datos del rango solicitado <br> Y mantiene formato CSV consistente <br> <br> **Escenario 3: Sin datos** <br> Dado que un usuario no tiene datos de consumo <br> Cuando se solicita exportación <br> Entonces el sistema retorna archivo CSV vacío con headers <br> Y retorna código 200 | EP-05 |
| TS-26 | Dashboard de Métricas para Técnicos | Como desarrollador, quiero crear endpoints para métricas de técnicos para proporcionar estadísticas básicas de servicios completados. | **Escenario 1: Obtener estadísticas básicas** <br> Dado que se envía GET /api/technicians/stats <br> Cuando se procesa la petición <br> Entonces el sistema cuenta servicios completados <br> Y calcula promedio de calificaciones <br> Y suma ingresos totales <br> Y retorna código 200 <br> <br> **Escenario 2: Estadísticas por período** <br> Dado que se envía GET /api/technicians/stats?period=monthly <br> Cuando se procesa la petición <br> Entonces el sistema agrupa datos por mes <br> Y calcula totales por período <br> Y retorna array de estadísticas mensuales <br> <br> **Escenario 3: Sin servicios** <br> Dado que un técnico no tiene servicios registrados <br> Cuando se solicitan estadísticas <br> Entonces el sistema retorna valores en cero <br> Y retorna código 200 | EP-06 |
| TS-27 | Verificador de Facturas Básico | Como desarrollador, quiero crear un endpoint para verificar cálculos de facturas para permitir a usuarios validar montos facturados. | **Escenario 1: Verificar cálculo simple** <br> Dado que se envía POST /api/bills/verify con datos de factura <br> Cuando se procesa la petición <br> Entonces el sistema calcula el monto esperado <br> Y compara con monto facturado <br> Y retorna resultado de verificación <br> Y retorna código 200 <br> <br> **Escenario 2: Detectar discrepancia** <br> Dado que existe diferencia entre cálculo y factura <br> Cuando se completa la verificación <br> Entonces el sistema marca como "discrepancia encontrada" <br> Y calcula la diferencia en monto <br> Y retorna detalles de la diferencia <br> <br> **Escenario 3: Cálculo correcto** <br> Dado que el cálculo coincide con lo facturado <br> Cuando se completa la verificación <br> Entonces el sistema marca como "correcto" <br> Y retorna confirmación de exactitud | EP-07 |
| TS-28 | Sistema de Alertas de Consumo | Como desarrollador, quiero crear endpoints para configurar alertas de consumo para notificar a usuarios cuando superen límites establecidos. | **Escenario 1: Configurar alerta** <br> Dado que se envía POST /api/alerts con umbral de consumo <br> Cuando se procesa la petición <br> Entonces el sistema almacena la configuración de alerta <br> Y activa el monitoreo para el usuario <br> Y retorna código 201 <br> <br> **Escenario 2: Evaluar alertas** <br> Dado que se registra nuevo consumo <br> Cuando se evalúan las alertas configuradas <br> Y el consumo supera el umbral <br> Entonces el sistema crea registro de alerta <br> Y marca para envío de notificación <br> <br> **Escenario 3: Consultar alertas** <br> Dado que se envía GET /api/alerts <br> Cuando se procesa la petición <br> Entonces el sistema retorna alertas del usuario <br> Y incluye estado de cada alerta <br> Y retorna código 200 | EP-05 |
| TS-29 | Análisis de Demanda por Zona | Como desarrollador, quiero crear endpoints para consultar estadísticas de servicios por zona para proporcionar información básica de demanda geográfica. | **Escenario 1: Estadísticas por zona** <br> Dado que se envía GET /api/services/stats/zones <br> Cuando se procesa la petición <br> Entonces el sistema cuenta servicios por zona geográfica <br> Y calcula totales por área <br> Y retorna estadísticas agrupadas <br> Y retorna código 200 <br> <br> **Escenario 2: Servicios más solicitados** <br> Dado que se envía GET /api/services/stats/popular <br> Cuando se procesa la petición <br> Entonces el sistema cuenta servicios por tipo <br> Y ordena por frecuencia <br> Y retorna lista ordenada <br> <br> **Escenario 3: Filtrar por período** <br> Dado que se envían parámetros de fecha <br> Cuando se calculan estadísticas <br> Entonces el sistema incluye solo datos del período <br> Y mantiene agrupación por zona | EP-06 |
| TS-30 | Sistema de Recomendaciones Básico | Como desarrollador, quiero crear un endpoint para sugerir servicios relevantes para mostrar recomendaciones simples basadas en historial del usuario. | **Escenario 1: Recomendaciones para propietarios** <br> Dado que se envía GET /api/recommendations/owner <br> Cuando se procesa la petición <br> Entonces el sistema busca servicios previos del usuario <br> Y sugiere servicios relacionados <br> Y retorna lista de recomendaciones <br> Y retorna código 200 <br> <br> **Escenario 2: Servicios populares** <br> Dado que un usuario no tiene historial <br> Cuando se solicitan recomendaciones <br> Entonces el sistema retorna servicios más populares <br> Y incluye breve descripción <br> <br> **Escenario 3: Recomendaciones para técnicos** <br> Dado que se envía GET /api/recommendations/technician <br> Cuando se procesa la petición <br> Entonces el sistema sugiere servicios rentables <br> Y recomienda zonas con demanda <br> Y retorna lista de oportunidades | EP-07 |
